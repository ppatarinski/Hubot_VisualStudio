...
#   hubot start build <build name> - Adds a build to the queue for the specified build type
#   hubot tc start build <buildType> of <project> - Adds a build to the queue for the specified build type of the specified project
...
module.exports = (robot) ->
  ...
  mapNameToIdForBuildType = (msg, project, name, callback) ->
    ...
    execute = (buildTypes) ->
      buildType =  _.find buildTypes, (bt) -> return bt.name.toLowerCase() == name.toLowerCase() and (not project? or bt.projectName == project)
      if buildType
        return buildType.id
